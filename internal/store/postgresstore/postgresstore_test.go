...
// newTestStore returns a Store backed by the Postgres instance named by
// NAGUS_TEST_POSTGRES_DSN, with the items table truncated for isolation. If
// the env var is unset, the test is skipped so the suite stays green on
...
func TestAttributesAndTokensRoundTrip(t *testing.T) {
	...
	// over its tokens, proving the ILIKE path sees the same data.
